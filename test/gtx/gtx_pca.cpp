...
int testEigenvectors(T epsilon)
{
	...
	std::vector<vec> testData;
	mat covarMat(ns_1aga::expectedCovarData());
	...
	if (!vectorEpsilonEqual(eigenvalues, vec(ns_1aga::expectedEigenvalues<D>()), epsilon))
		return failReport(__LINE__);

	for (int i = 0; i < D; ++i)
	{
		vec act = glm::normalize(eigenvectors[i]);
		vec exp = glm::normalize(ns_1aga::expectedEigenvectors<D>()[i]);
